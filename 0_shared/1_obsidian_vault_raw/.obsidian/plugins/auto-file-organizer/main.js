...
var AutoFileOrganizer = class extends import_obsidian.Plugin {
  async onload() {
...
var AutoFileOrganizerSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  async display() {
    ...
    for (const [folder1, folder2] of Object.entries(this.plugin.settings.blackList)) {
      new import_obsidian.Setting(containerEl).setName(`Exclude Folder`).setDesc("Change the folder for this tag").addDropdown((dropdown) => {
        dropdown.addOption("", "Select folder...");
        allFolders.forEach((f) => dropdown.addOption(f.path, f.path));
    ...
    let blackList = "";
    new import_obsidian.Setting(containerEl).setName("Set Black Folder List").setDesc("Indicate what folder is excluded for automatically get tag mapping").addDropdown((dropdown) => {
      dropdown.addOption("", "Select folder...");
      allFolders.forEach((folder) => dropdown.addOption(folder.path, folder.path));
